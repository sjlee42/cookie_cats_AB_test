...
**분석 대상:** D7 리텐션 (Primary KPI), D1 리텐션·게임 라운드 수 (Secondary KPI)
...
| # | 제목 | 주요 내용 |
|---|---|---|
| 1 | 데이터 불러오기 & 전처리 | CSV 로드, 0라운드·이상치 제거, 폰트 설정 |
| 2 | 기술 통계 — 분할표 | D7 retention 분할표, 그룹별 비율 |
| 3 | 카이제곱 검정 | D7 독립성 검정, 기대빈도 확인 |
| 4 | Two-proportion Z-test | 비율 차이 정규근사 + 95% CI 산출, 표준정규분포 시각화 |
| 5 | Bootstrap 검정 | D7 10,000회 재표본, 신뢰구간·p-value |
| 6 | 순열 검정 | D7 레이블 셔플 귀무분포 구성, p-value |
| 7 | Secondary KPI — D1 잔존율 | D1 분할표, 카이제곱 + Bootstrap |
| 8 | Secondary KPI — 게임 라운드 수 | Mann-Whitney U, boxplot + 밀도 히스토그램 |
| 9 | 효과 크기 & 검정력 | Cohen's h (D7/D1), Statistical Power, 시각화 |
| 10 | 검정 결과 종합 | 전 KPI × 전 검정 결과 통합 출력, 비즈니스 결론 |

#### D7 잔존율 검정 결과 (Primary KPI)
...
| 순열 검정 (10,000회) | diff = +0.8103%p | 0.0023 | 기각 | — |

#### Secondary KPI 결과

| KPI | 검정 | p-value | 결론 |
|---|---|---|---|
| D1 잔존율 | 카이제곱 + Bootstrap | p = 0.116 / 0.1155 | H0 채택 (유의하지 않음) |
| 게임 라운드 수 | Mann-Whitney U | p = 0.1177 | H0 채택 (유의하지 않음) |

#### 효과 크기 & 검정력

| KPI | Cohen's h | 해석 | 검정력 |
|---|---|---|---|
| D7 잔존율 | 0.0205 | Small (매우 작음) | 85.22% ✅ |
| D1 잔존율 | 0.0108 | Small (매우 작음) | 37.26% (D1 차이 자체가 미미) |

> **결론:** 4가지 검정 모두 D7 H0 기각. gate_30 D7 리텐션(19.84%)이 gate_40(19.03%)보다 통계적으로 유의미하게 높다.
> D1·라운드 차이는 유의하지 않음. 효과 크기는 매우 작음(Cohen's h ≈ 0.02).
...
| `02_가설검정.ipynb` | ✅ 완료 | 카이제곱 / Z-test / Bootstrap / 순열 검정 + D1·라운드 Secondary KPI + Cohen's h & Power |
...
- 결론은 검정 방법(4종: 카이제곱·Z-test·Bootstrap·순열) 및 이상치 제거 기준(3종) 모두에서 일관되게 재현됨
- D1 잔존율·게임 라운드 수 차이는 유의하지 않음. 효과 크기 Cohen's h = 0.02 (매우 작음), 검정력 85.2% (D7 기준)
