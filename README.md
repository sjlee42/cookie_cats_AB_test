...
- **전처리 하나로 결론이 정반대로 뒤집히는 경험이 충격적이었다.** "게이트를 경험한 유저만 분석하는 것이 더 정확하지 않을까?"라는 직관이 생존 편향이라는 함정을 만든다는 것을 배웠다. A/B 테스트에서는 ITT 원칙을 먼저 따르고, Per-Protocol은 탐색적 보조 분석으로만 활용해야 한다.
