...
A/B 그룹 균형 검증        Two-proportion Z-test       변화시켜 강건성 검증        생존 편향 분석
분포 파악                Bootstrap 10,000회                                    인과적 해석 vs 상관 해석
잔존율 초기 비교          순열 검정 10,000회
                        Mann-Whitney U
                        Cohen's h 효과 크기
                        검정력 분석
```
...
| Two-proportion Z-test | D7 잔존율 | 신뢰구간을 직접 산출하여 효과 방향·크기를 정량화 |
| Bootstrap (10,000회) | D1, D7 잔존율 | 분포 가정 없이 강건한 신뢰구간 추정. Chi-square의 모수적 가정을 보완 |
| 순열 검정 (10,000회) | D7 잔존율 | 레이블 셔플로 귀무분포를 직접 시뮬레이션. 가장 가정이 적은 비모수 검정 |
...
| KPI | 결과 | 판정 |
|-----|------|------|
| **D7 잔존율** (Primary) | gate_30이 +0.81%p 높음, p < 0.003 (4가지 검정 일관) | ✅ 유의 |
| D1 잔존율 | gate_30이 +0.54%p 높음, p = 0.116 | ❌ 유의하지 않음 |
| 게임 라운드 (Mann-Whitney U) | 중앙값 동일 (18.0), p = 0.118 | ❌ 유의하지 않음 |
| 효과 크기 (Cohen's h) | D7: h = 0.02, D1: h = 0.01 (매우 작음) | ⚠️ 실질 효과 제한적 |
| 검정력 | D7: 85.2% ✅ / D1: 37.3% (차이 자체가 미미) | D7 충분 |
| 민감도 | 4가지 기준 모두 동일 결론 | ✅ 강건함 |

### D7 잔존율 검정 결과 (4가지 방법 모두 H0 기각)

| 검정 방법 | 통계량 | p-value | 95% CI |
|----------|--------|:-------:|--------|
| 카이제곱 | χ² = 8.9849 | 0.0027 | — |
| Two-proportion Z-test | Z = 3.0061 | 0.0026 | [+0.28%p, +1.34%p] |
| Bootstrap (10,000회) | diff = +0.8103%p | 0.0033 | [+0.27%p, +1.35%p] |
| 순열 검정 (10,000회) | diff = +0.8103%p | 0.0023 | — |

### 최종 권고: Gate 30 유지 (현행 유지)

D7 잔존율에서 gate_30이 통계적으로 유의하게 높다 (p < 0.003, 4가지 검정 일관). gate_40으로 변경 시 소폭 하락만 발생하며, 어떤 검정·이상치 기준으로 분석해도 gate_40의 장점은 발견되지 않는다. 효과 크기가 극히 작으므로(0.81%p, Cohen's h = 0.02), 잔존율의 의미 있는 개선을 위해서는 게이트 위치가 아닌 다른 변수의 후속 실험을 설계하는 것을 권고한다.
